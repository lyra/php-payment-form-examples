...
if (! class_exists('Request', false)) {
    class Request
    {
        ...
        {
            // Initialize encoding.
            $this->encoding = in_array(strtoupper($encoding), Api::$SUPPORTED_ENCODINGS)
            ...
            $timestamp = time();
            $this->set('vads_trans_id', Api::generateTransId($timestamp));
        ...
        {
            $this->requestParameters[$name] = new Field($name, $label, $regex, $required, $length);
        ...
        public function set($name, $value)
        {
            ...
            if ($name == 'vads_key_test') {
                return $this->setKey($value, 'TEST');
            } elseif ($name == 'vads_key_prod') {
                return $this->setKey($value, 'PRODUCTION');
            } elseif ($name == 'vads_platform_url') {
        ...
        public function addProduct($label, $amount, $qty, $ref, $type = null, $vat = null)
        {
            ...
            $ok &= $this->addField('product_label' . $index, 'Product label', '#^[^<>"+-]{0,255}$#u', false, 255, $label);
            $ok &= $this->addField('product_amount' . $index, 'Product amount', '#^[1-9]\d*$#u', false, 12, $amount);
            $ok &= $this->addField('product_qty' . $index, 'Product quantity', '#^[1-9]\d*$#u', false, 255, $qty);
            $ok &= $this->addField('product_ref' . $index, 'Product reference', '#^[A-Za-z0-9]{0,64}$#u', false, 64, $ref);
            $ok &= $this->addField('product_type' . $index, 'Product type', '#^' . implode('|', self::$ACCORD_CATEGORIES ) . '$#u', false, 30, $type);
            $ok &= $this->addField('product_vat' . $index, 'Product tax rate', '#^((\d{1,12})|(\d{1,2}\.\d{1,4}))$#u', false, 12, $vat);
        ...
        private function generateSignature($fields, $hashed = true)
        {
            ...
            return Api::sign($params, $this->getKey(), $this->algo, $hashed);
        }
