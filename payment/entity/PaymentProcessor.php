...
class PaymentProcessor
{
    ...
    public function submitStandardPaymentForm($order_info) {
        ...
        $target = ($request->get('action _mode') === 'IFRAME') ? 'target="payframe"' : '';
...
EOT;
    ...
    public function submitMultiPaymentForm($order_info, $params_multi) {
        ...
        $answer = $request->setMultiPayment(null /* use already set amount */, $first, $params_multi['count'], $params_multi['period']);
        ...
        //$target = (strcmp($request->get('action _mode'), 'IFRAME')) ? 'target="payframe"' : '';
